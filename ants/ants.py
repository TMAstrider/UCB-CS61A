...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        place_temp = self.place
        cnt = 0
        while place_temp is not beehive:
            # pass
            tmp = rANTdom_else_none(place_temp.bees)
            if tmp != None and (self.min_range <= cnt <= self.max_range):
                return tmp
            place_temp = place_temp.entrance
            cnt += 1
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if amount >= self.armor :
            damage_tmp = amount + self.damage
        else:
            damage_tmp = amount
        for i in self.place.bees[:]:
            Insect.reduce_armor(i, damage_tmp)
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        # BEGIN Problem 8
        "*** YOUR CODE HERE ***"
        Place.add_insect(self, insect)    # use the method that had been already built above -> ! !
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        "*** YOUR CODE HERE ***"
        if  QueenAnt.num == 0:
            self.ID_of_queen = True
            QueenAnt.num += 1
        else :
            self.ID_of_queen = False
            QueenAnt.num += 1
