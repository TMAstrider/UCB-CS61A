...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    # torque_right = len(right(m)) * total_weight(end(right(m)))
    # torque_left = len(left(m)) * total_weight(end(left(m)))
    # print('DEBUG', torque_left)
    # print('DEBUG', torque_right)
    if is_planet(m):
        return True
...
def replace_leaf(t, find_value, replace_value):
    ...
    if is_leaf(t) :
        ...
        return t
    for i in branches(t):
        releaf += [replace_leaf(i, find_value, replace_value)]
    return tree(label(t), releaf)
